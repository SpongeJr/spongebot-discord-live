...
/*
var	SubCommand = function(trigger, config) {
		let moduleName = config.moduleName || trigger;
		let defaults = {
			disabled: false,
			help: commands[trigger].help,
			longHelp: cons.MODULES[moduleName].commands[trigger].longHelp,
			do: function(message, args) {
				cons.MODULES[moduleName].commands[trigger].do(message, args);
			},
...
module.exports = {
	Command: function(trigger, config, MODULES, BOT) {

		let moduleName = config.moduleName || trigger;
		console.log(`* ADDING Command:${trigger.padStart(14, ' ')} from module ${moduleName}`);
		let defaults = {
			subCmd: {},
			disabled: false,
			help: MODULES[moduleName].commands[trigger].help,
			longHelp: MODULES[moduleName].commands[trigger].longHelp,
			do: (message, args, gameStats, bankroll, BOT) => {

				args = args.split(' ');

				if (args[0] === '') {
					//trigger all by itself
					MODULES[moduleName].commands[trigger].do(message, args, gameStats, bankroll, BOT);
				} else {
					let possibleSubCmd = args[0].toLowerCase();

					if (MODULES[moduleName].commands[trigger].subCmd.hasOwnProperty(possibleSubCmd)) {
						//we've found a found sub-command, so do it...
						MODULES[moduleName].commands[trigger].subCmd[possibleSubCmd].do(message, args, gameStats, bankroll, BOT);
						return; // we're done here
					} else {
						// no sub-command found, but we have something to pass along as args
						MODULES[moduleName].commands[trigger].do(message, args, gameStats, bankroll, BOT);
					}
				}
			},
	...
	getStat: function(who, game, stat, gameStats) {
		...
		if (!gameStats) {
			console.log('!!! getStat: No gameStats data was passed to me, you probably forgot gameStats!');
			return;
		}
	...
	chSend: function(message, str, emb) {
		...
		if (str.length > 1990) {
			this.debugPrint(`chSend: Truncating ${str.length} length message to 1990 chars.`);
			str = str.substring(0, 1990);
		}
	...
	idToNick: function(inp, gameStats) {
		// resolves id's sent in to a gamestats.nick if one exists, else returns what it was sent

		let result = this.getStat(inp, "profile", "nick", gameStats);
