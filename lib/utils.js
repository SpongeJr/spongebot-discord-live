...
module.exports = {
	Command: function(trigger, config, MODULES) {
		...
		let defaults = {
			...
			do: (message, args, gameStats, bankroll) => {
			
				args = args.split(' ');
				
				if (args[0] === '') {
					//trigger all by itself
					MODULES[moduleName].commands[trigger].do(message, args, gameStats, bankroll);					
				} else {
					args[0] = args[0].toLowerCase();
					if (MODULES[moduleName].commands[trigger].subCmd.hasOwnProperty(args[0])) {
						//we've found a found sub-command, so do it...
						MODULES[moduleName].commands[trigger].subCmd[args[0]].do(message, args, gameStats, bankroll);
